...
X = scaler.fit_transform(cleaned_df[log_features])
...
plt.title('Simple Clusters on Log GSF vs. Log EUI')
...
plt.title('Complex Clusters on Log GSF vs. Log EUI')
...
plt.title('Simple Clusters on GSF vs. Site EUI')
plt.xlabel('Gross Floor Area (Sq Ft)')
plt.ylabel('Site EUI (kBtu/sf)')
plt.legend(title='Cluster')
plt.grid(True)
...
for ax, metric in zip(axs, features):
    ...
    for cluster in sorted(cleaned_df[cluster_column].unique()):
        # filter data for the cluster
        cluster_data = cleaned_df[cleaned_df[cluster_column] == cluster][metric]
...
plt.tight_layout(rect=(0.0, 0.03, 1.0, 0.95))  # Adjust layout for title
# plt.show()
...
cleaned_df['Cluster_2'] = labels_simple
cleaned_df[f'Cluster_{optimal_clusters_complex}'] = labels_complex

# Merge cluster labels back to the original dataset
# This keeps all columns, including the ones not used in clustering
original_df['Cluster_2'] = cleaned_df['Cluster_2']
original_df[f'Cluster_{optimal_clusters_complex}'] = cleaned_df[f'Cluster_{optimal_clusters_complex}']
